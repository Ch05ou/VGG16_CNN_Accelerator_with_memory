...
//       
//       Creation Date:  Sun Dec 15 17:55:26 2024
//       Version: 	r8p2
//
...
`ifdef POWER_PINS
module kernel_rf (VDDCE, VDDPE, VSSE, CENY, WENY, AY, DY, Q, CLK, CEN, WEN, A, D, EMA,
    EMAW, EMAS, TEN, BEN, TCEN, TWEN, TA, TD, TQ, RET1N, STOV);
`else
module kernel_rf (CENY, WENY, AY, DY, Q, CLK, CEN, WEN, A, D, EMA, EMAW, EMAS, TEN,
 ...
 wire  CLK_;
  ...
  assign DY_ = RET1N_ ? (TEN_ ? D_ : TD_) : {16{1'bx}};
   `ifdef ARM_FAULT_MODELING
     kernel_rf_error_injection u1(.CLK(CLK_), .Q_out(Q_), .A(A_int), .CEN(CEN_int), .TQ(TQ_), .BEN(BEN_), .WEN(WEN_int), .Q_in(Q_int));
  `else
...
`endif
`timescale 1ns/1ps
module kernel_rf_error_injection (Q_out, Q_in, CLK, A, CEN, WEN, BEN, TQ);
   output [15:0] Q_out;
   input [15:0] Q_in;
   input CLK;
   input [8:0] A;
